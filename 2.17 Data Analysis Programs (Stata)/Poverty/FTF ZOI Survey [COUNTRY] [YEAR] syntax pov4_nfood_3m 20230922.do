...
consumption expenditure in the 3 months preceding the survey. This is the 4th 
...
//Log result:  $analtyic\Log\FTF ZOI Survey [COUNTRY] [YEAR] pov4_nfood_3m.log
//Output data: $analytic\Results\FTF ZOI Survey [COUNTRY] [YEAR] pov4_nfood_3m.dta
//Syntax:      $syntax\FTF ZOI Survey [COUNTRY] [YEAR] syntax pov4_nfood_3m.do 
 
cap log close
cap log using  "$analytic\log\FTF ZOI Survey [Country] [Year] pov4_nfood_1w.log", replace
********************************************************************************
...
keep hhea hhnum c05 c06 hhsize_dj v840* v8700r v8100r     //keep relevant vars only
...
********************************************************************************
**1a. Flag households if none of the items were purchased in the past 3 months  
*     [v8402>=2 or v8402=.]

sum   v8402*
quiet for var v8402*: replace X=. if X>2  
egen  v8402min=rowmin(v8402*)
tab   v8402min,m                         //HHs missing info on consumption expenditure on all items

sum   v8402* if v8402min==.   //HH is missing answer on all items
sum   v8402* if v8402min==2  //HH answer is "no" to all items 

**1b. Drop all flagged households from the working dataset. These households 
**    are added back into the dataset later in step 6.  Assuming it is  
**	  legitimate that these households did not make purchases on these items  
**    in the past 3 months, their PCD expenditures are set to 0 
**    later in  step 9.
...
reshape long v8401_ v8402_ v8403_ , i(hhea hhnum c05 c06 hhsize_dj) j(j) string
des
rename *_ *
sum    v840*
tab    v8401,m
...
clonevar item=v8401
gen      totnfood_item=v8403  //total nonfood expenditure on [item]

**1f. Drop expenditure on lodging in rest house or hotel in past 3 months
**    (item 8275 in core questionnaire) that are lumpy expenditures but are
**	  asked to respondents.  

drop if item==8275

**1g. Check the expenditure values for each item and set invalid values to missing.
bys item: tab totnfood_item  
replace totnfood_item = . if totnfood_item> 999999997  

**Label variable name
la var item          "Non-food item, past 3 months" 
la var v8402         "Did HH purchase/pay for any item, past 3 months"
la var totnfood_item "Total expenditures on non-food item, past 3 months"  

********************************************************************************
//STEP 2: CALCULATE PCD NONFOOD CONSUMPTION EXP ON EACH ITEM PURCHASED IN THE 
**        PAST 3 MONTHS 
********************************************************************************
gen      pcdnfood_item = (totnfood_item/hhsize_dj)/90 if totnfood!=.
tabulate item, sum(pcdnfood_item) 
la var   pcdnfood_item "PCD nonfood consumption exp on item, past 3 months"

********************************************************************************
//STEP 3: IDENTIFY POTENTIAL OUTLIERS of pcdnfood_item AND SET CONFIRMED 
**		  OUTLIERS TO MISSING 
********************************************************************************
...
*    file: "4a_pcdnfood_item_outliers.xlsx"
...
export excel "$analytic\Log\4a_pcdnfood_item_outliers.xlsx", replace first(variable)
restore
...
********************************************************************************
//STEP 4: CALCULATE LOCAL MEDIAN OF pcdnfood_item AND REPLACE OUTLIERS
**		   WITH MEDIAN
********************************************************************************
***Examine outliers one by one for plausibility before replacing them with 
** the median.
*drop tot* impute
...
********************************************************************************
//HH non-food consumption in the past 3 months
...
********************************************************************************
//STEP 7: IDENTIFY POTENTIAL OUTLIERS OF pcdnfood_hh AND SET VERIFIED OUTLIERS 
**		  TO MISSING
********************************************************************************
...
*    file: "4b_pcdnfood_hh_outliers.xlsx"
*    Open the Excel file and examine each outlier one by one for plausibility.
...
export excel "$analytic\Log\4b_pcdnfood_hh_outliers.xlsx", replace first(variable)
restore
...
********************************************************************************
//STEP 9: SET pcdnfood_hh TO 0 IF STILL MISSING. 
********************************************************************************
** A missing value means that the household either did not purchase any of these
** items in the past 3 months or the household is missing consumption 
** information.
...
********************************************************************************
rename pcdnfood_hh pcd_nfood_3m

la var  pcd_nfood_3m "PCD non-food consumption expenditure on item, past 3 months"

order  hhea hhnum pcd_nfood_3m
keep   hhea hhnum pcd_nfood_3m
des
sort   hhea hhnum

disp "DateTime: $S_DATE $S_TIME"

la data "Per capita daily non-food consumption exp by HH, past 3 months"
save    "$analytic\results\FTF ZOI Survey [Country] [Year] pov4_nfood_3m.dta", replace 
